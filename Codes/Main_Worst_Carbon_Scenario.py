# #This file generates the worst practices for AI server carbon emission
# import numpy as np
# import math
# import csv
...
# Make sure this filename matches Data/Spatial Distribution
frozen_data = np.loadtxt(
    DATA_DIR / "Spatial Distribution" / "CUE_last25_spatial.txt",
    delimiter="\t",
    dtype="float",
)
...
for i in range(tem_sce_num):
    for j in range(spt_sce_num):
        for k in range(typ_sce_num):
            # import unit emission & water data
            name = DATA_DIR / "Grid Factor" / f"cases_REcon_{i+1}_CUE_last25_CF.txt"
            e_data = np.loadtxt(name, delimiter=" ", dtype="float")
            name = DATA_DIR / "Grid Factor" / f"cases_REcon_{i+1}_CUE_last25_WF.txt"
            w_data = np.loadtxt(name, delimiter=" ", dtype="float")
            emission_data = np.zeros([L_1, L_2])
            water_data = np.zeros([L_1, L_2])
            if i == 0:
            ...
            with open(DATA_DIR / "Worst_PUE.csv", newline="") as csvfile:
                spamreader = csv.reader(csvfile, delimiter=",", quotechar="|")
                for row in spamreader:
                    if flag == 0:
                        flag = flag + 1
...
# ---------- Save annual worst-carbon-scenario totals (2024–2030) ----------
...
out_path = output_dir / "worst_carbon_totals.csv"
...
print(f"Worst-carbon scenario annual totals saved to {out_path}")
