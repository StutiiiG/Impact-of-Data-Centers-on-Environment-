# # This file generates the best practices for AI server carbon reduction
# import numpy as np
# import math
# import csv
...
# utilization_level_1=0.3*0.76+0.7*0.45
...
# frozen_data=np.loadtxt(r'D:\2023 Fall\NS R1 Files\Inputs\Spatial Dirstribution\CUE_25_spatial.txt',delimiter='\t',dtype='float')  
...
#             name=r'D:\2023 Fall\NS R1 Files\Inputs\Grid Factor\cases_REadv_'+repr(i+1)+'_CUE_25_CF.txt'
#             e_data=np.loadtxt(name,delimiter=' ',dtype='float')
#             name=r'D:\2023 Fall\NS R1 Files\Inputs\Grid Factor\cases_REadv_'+repr(i+1)+'_CUE_25_WF.txt'
...
#             flag=0
#             with open(r'D:\2023 Fall\NS R1 Files\Inputs\Best_PUE.csv', newline='') as csvfile:
...
# Make sure the filename here matches what's in Data/Spatial Distribution/
frozen_data = np.loadtxt(
    DATA_DIR / "Spatial Distribution" / "CUE_25_spatial.txt",
    delimiter="\t",
    dtype="float",
)
...
for i in range(tem_sce_num):
    for j in range(spt_sce_num):
        for k in range(typ_sce_num):
            # import unit emission & water data (CF/WF)
            name = DATA_DIR / "Grid Factor" / f"cases_REadv_{i+1}_CUE_25_CF.txt"
            e_data = np.loadtxt(name, delimiter=" ", dtype="float")
            name = DATA_DIR / "Grid Factor" / f"cases_REadv_{i+1}_CUE_25_WF.txt"
            w_data = np.loadtxt(name, delimiter=" ", dtype="float")
            ...
            # Best_PUE.csv
            with open(DATA_DIR / "Best_PUE.csv", newline="") as csvfile:
                spamreader = csv.reader(csvfile, delimiter=",", quotechar="|")
                for row in spamreader:
                    if flag == 0:
                        flag = flag + 1
...
# ---------- Save annual best-carbon-scenario totals (2024–2030) ----------
...
out_path = output_dir / "best_carbon_totals.csv"
...
print(f"Best-carbon scenario annual totals saved to {out_path}")
